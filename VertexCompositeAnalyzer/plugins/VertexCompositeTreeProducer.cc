		...
		*/
		...
		{ // doGenMatching
			...
			for(unsigned genPair=0; genPair<genpars->size(); ++genPair){ //loop over all gen particles -> to find known D0 to kPi pairs 
				...
				cout << "gen_d1->charge() = " <<  gen_d1->charge() << " and gen_d2->charge()= " << gen_d2->charge() << endl;
				cout << " ------------------------------- " << endl;
				cout << "reco_d1->pt() = " <<  reco_d1->pt() << " and reco_d2->pt()= " << reco_d2->pt() << endl;
				cout << "gen_d1->pt() = " <<  gen_d1->pt() << " and gen_d2->pt()= " << gen_d2->pt() << endl;
				*/

				if(!threeProngDecay_ && !(fabs(gen_d1->pdgId())==PID_dau1_ && fabs(gen_d2->pdgId())==PID_dau2_) && !(fabs(gen_d2->pdgId())==PID_dau1_ && fabs(gen_d1->pdgId())==PID_dau2_)) continue; //make sure k pi pairs              
				...
				/*
					...
					cout << "-- this is from else -- " << endl;
					cout << "reco_d1->pdgId() = " <<  reco_d1->pdgId() << " and reco_d2->pdgId()= " << reco_d2->pdgId() << endl;
					cout << "gen_d1->pdgId() = " <<  gen_d1->pdgId() << " and gen_d2->pdgId()= " << gen_d2->pdgId() << endl;
				}
					*/
		...
		if(doGenMatchingTOF_)
		{
			for(unsigned it=0; it<genpars->size(); ++it){
				...
				if(fabs(id)!=PID_ && trk.charge())
				{
					...
					if(deltaR < deltaR_ && fabs((trk.pt()-pt1[it])/pt1[it]) < 0.2 && trk.charge()==charge1[it] && pid1[it]==-99999)
					{
						pid1[it] = id;
					} 
					...
					if(deltaR < deltaR_ && fabs((trk.pt()-pt2[it])/pt2[it]) < 0.2 && trk.charge()==charge2[it] && pid2[it]==-99999)
					{
						pid2[it] = id;
					}
				}
				...
				if(fabs(id)==PID_ && trk.numberOfDaughters()==2)
				{
					...
					TVector3 d1vect(gen_d1->px(),gen_d1->py(),gen_d1->pz());
					TVector3 d2vect(gen_d2->px(),gen_d2->py(),gen_d2->pz());
					int id1 = gen_d1->pdgId();
					int id2 = gen_d2->pdgId();

					double deltaR = d1vect.DeltaR(dauvec1);
					if(deltaR < deltaR_ && fabs((gen_d1->pt()-pt1[it])/pt1[it]) < 0.2 && gen_d1->charge()==charge1[it] && pid1[it]==-99999)
					{
						pid1[it] = id1;
					}
					deltaR = d2vect.DeltaR(dauvec1);
					if(deltaR < deltaR_ && fabs((gen_d2->pt()-pt1[it])/pt1[it]) < 0.2 && gen_d2->charge()==charge1[it] && pid1[it]==-99999)
					{
						pid1[it] = id1;
					}

					deltaR = d1vect.DeltaR(dauvec2);
					if(deltaR < deltaR_ && fabs((gen_d1->pt()-pt2[it])/pt2[it]) < 0.2 && gen_d1->charge()==charge2[it] && pid2[it]==-99999)
					{
						pid2[it] = id2;
					}
					deltaR = d2vect.DeltaR(dauvec2);
					if(deltaR < deltaR_ && fabs((gen_d2->pt()-pt2[it])/pt2[it]) < 0.2 && gen_d2->charge()==charge2[it] && pid2[it]==-99999)
					{
						pid2[it] = id2;
					}
				}
		...
		if(twoLayerDecay_)
		{
			grand_mass[it] = reco_d1->mass();

			const reco::Candidate * gd1 = reco_d1->daughter(0);
			const reco::Candidate * gd2 = reco_d1->daughter(1);
			...
			//vertexCovariance 00-xError 11-y 22-z
			secvz = reco_d1->vz(); secvx = reco_d1->vx(); secvy = reco_d1->vy();

			//abby reco::Vertex secvtx = d1->vertex();
			...
			//DCA
			math::XYZPoint bestsectx(bestvx,bestvy,bestvz);
			...
			//vtxChi2
			grand_vtxChi2[it] = reco_d1->vertexChi2();
			grand_ndf[it] = reco_d1->vertexNdof();
			...
			TVector3 secvec(reco_d1->px(),reco_d1->py(),reco_d1->pz());

			TVector3 ptosvec2D(secvx-bestvx,secvy-bestvy,0);
			TVector3 secvec2D(reco_d1->px(),reco_d1->py(),0);
			...
			SMatrixSym3D totalCov = vtx.covariance() + reco_d1->vertexCovariance();
			SVector3 distanceVector(secvx-bestvx,secvy-bestvy,secvz-bestvz);
			...
			//Decay length 2D
			SVector6 v1(vtx.covariance(0,0), vtx.covariance(0,1),vtx.covariance(1,1),0,0,0);
			SVector6 v2(reco_d1->vertexCovariance(0,0), reco_d1->vertexCovariance(0,1),reco_d1->vertexCovariance(1,1),0,0,0);
